...
class Walk():
    ...
    FOOT_STEP_HEIGHT = 25   # the height of the stepping foot
    FOOT_STEP_WIDTH  = 80   # the width of the stepping foot
    Y_OFFSET = -16            # the body y offset
    FOOT_STAND_OFFSET = 20  # the foot center offset
    Z_ORIGIN = 80
    ...
    def get_coords(self):
        ...
        for section in range(self.SECTIONS):
            for step in range(self.STEPS):
                ...
                foot_coords.append((foot_coord, rpy, pos))
        return foot_coords
...
def test():
    ...
    # try:
    while True:
        for foot_coord, rpy, pos in foot_coords:
            print(foot_coord)
            # dog.set_rpy(**rpy)
            # dog.set_pose(**pos)
